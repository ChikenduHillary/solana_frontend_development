...
const Starter = () => {
    ...
    const fundWallet = async (event: { preventDefault: () => void }) => {
        ...
        try {
            ...
            const signature = await sendTransaction(transaction, connection, { signers: [sender] });
            setTxSig(signature);
    ...
    return (
        <main className='max-w-7xl grid grid-cols-1 sm:grid-cols-6 gap-4 p-4 text-white'>
            <form onSubmit={fundWallet} className='rounded-lg min-h-content bg-[#2a302f] p-4 sm:col-span-6 lg:col-start-2 lg:col-end-6'>
